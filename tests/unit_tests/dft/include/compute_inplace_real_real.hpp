...
int DFT_Test<precision, domain>::test_in_place_real_real_USM() {
    static_assert(domain != oneapi::mkl::dft::domain::REAL,
                  "real to complex REAL_REAL transforms are not supported");
    ...
    oneapi::mkl::dft::compute_backward<std::remove_reference_t<decltype(descriptor)>,
                                       PrecisionType>(descriptor, inout_re.data(), inout_im.data(),
                                                      dependencies)
        .wait();
    ...
    EXPECT_TRUE(check_equal_vector(output_data.data(), input.data(), input.size(), abs_error_margin,
